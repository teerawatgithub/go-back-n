...
main (int argc, char *argv[])
{
  FILE* fp = NULL;
  int datasize = 0;
  char buffer[300000];		/* buffer with room for null byte */
  buffer[300000] = '\0';		/* null terminate the buffer */
  int sock;			/* Socket */
  ...
  bzero (buffer,300000 );		/* zero out the buffer */

  if (argc < 3 || argc > 5)		/* Test for correct number of parameters */
    {
      fprintf (stderr, "Usage:  %s <UDP SERVER PORT> <CHUNK SIZE> [<LOSS RATE>] <NAME OUTPUT>\n", argv[0]);
      exit (1);
    }
  ...
  for (;;)			/* Run forever */
	...
	//	  free(buffer);
	  ...
	  if(!fwrite(buffer, datasize,1,fp))
	  {
	//	  free(buffer);
		  fclose(fp);
		  return 0;
